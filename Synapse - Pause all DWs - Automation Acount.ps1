    ...
    if (!$isServerSynapseWorkspace) {
        ...
        foreach ($SynapseSqlPool in $allSynapseSqlPools) {
            ...
            elseif ($SynapseSqlPool.Status -eq "Online") {
                ...
                # Pause SQL Pool
                $startTimePause = Get-Date
                Write-Output  "  -> Pausing SQL Pool [$($SynapseSqlPool.DatabaseName)]"

                if (!$debug) {
                    $resultsynapseSqlPool = $SynapseSqlPool | Suspend-AzSqlDatabase
                }

                # Show that the SQL Pool has been pause and how long it took
                $endTimePause = Get-Date
                $durationPause = NEW-TIMESPAN –Start $startTimePause –End $endTimePause

                if ($resultsynapseSqlPool.Status -eq "Paused") {
                    Write-Output "  -> SQL Pool [$($resultsynapseSqlPool.DatabaseName)] paused in $($durationPause.Hours) hours, $($durationPause.Minutes) minutes and $($durationPause.Seconds) seconds. Current status [$($resultsynapseSqlPool.Status)]"
                }
                else {
                    if (!$debug) {
                        $iErrorCount += 1;
                        Write-Error "  -> (resultsynapseSqlPool.Status -eq ""Paused"") SQL Pool [$($resultsynapseSqlPool.DatabaseName)] paused in $($durationPause.Hours) hours, $($durationPause.Minutes) minutes and $($durationPause.Seconds) seconds. Current status [$($resultsynapseSqlPool.Status)]"
                    }
                    else {
                        Write-Host "This is a debug session - Nothing was done" -ForegroundColor Yellow
                    }
